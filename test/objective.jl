@testset "Objective" begin
    ...
    @test DirectTrajectoryOptimization.eval_obj(obj, X, U) - sum([ft(X[t], U[t]) for t = 1:T-1]) - fT(X[T], U[T]) ≈ 0.0
    DirectTrajectoryOptimization.eval_obj_grad!(grad, idx_xu, obj, X, U) 
    @test norm(grad - vcat([[2.0 * x1; 0.2 * u1] for t = 1:T-1]..., 20.0 * x1)) < 1.0e-8

    info = @benchmark DirectTrajectoryOptimization.eval_obj($obj, $X, $U)
    info = @benchmark DirectTrajectoryOptimization.eval_obj_grad!($grad, $idx_xu, $obj, $X, $U)
end