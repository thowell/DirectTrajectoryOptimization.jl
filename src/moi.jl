...
function MOI.eval_constraint(p::Problem{T}, con, z) where T
    ...
    !isempty(p.idx.stage_con) && eval_con!(con, p.idx.stage_con, p.trajopt.con.stage, p.trajopt.x, p.trajopt.u)
end
...
function MOI.eval_constraint_jacobian(p::Problem{T}, jac, z) where T
    ...
    !isempty(p.idx.stage_jac) && eval_jac!(jac, p.idx.stage_jac, p.trajopt.con.stage, p.trajopt.x, p.trajopt.u)
    return nothing
end
...
function MOI.eval_hessian_lagrangian(p::MOI.AbstractNLPEvaluator, hess, z, σ, λ)
    ...
    eval_obj_hess!(hess, p.idx.obj_hess, p.trajopt.obj, p.trajopt.x, p.trajopt.u, σ)
    eval_hess_lag!(hess, p.idx.dyn_hess, p.trajopt.model.dyn, p.trajopt.x, p.trajopt.u, p.trajopt.w, p.trajopt.λ_dyn)
    eval_hess_lag!(hess, p.idx.stage_hess, p.trajopt.con.stage, p.trajopt.x, p.trajopt.u, p.trajopt.λ_stage)
end
